...
def _collect_daily() -> int:
    ...
    注意：daily_news_skill模块不存在，此函数暂时返回0
    """
    print("[daily] daily_news_skill模块不存在，跳过采集")
    return 0
...
def collect_all_news() -> Dict[str, int]:
    ...
    tasks = [
        ...
        # (_collect_daily, "daily"),  # daily_news_skill模块不存在，暂时移除
    ]
...
SECTION_COLLECTORS = {
    ...
    # "daily": _collect_daily,  # daily_news_skill模块不存在，暂时移除
}
