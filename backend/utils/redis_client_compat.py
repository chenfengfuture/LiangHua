...
NEWS_PENDING_FINBERT_KEY = RedisServiceBase.NEWS_PENDING_FINBERT_KEY
...
def pending_finbert_push(news_id: int, table_name: str = None) -> bool:
    """将已完成 LLM 分析的新闻 ID 推入 news:pending_finbert List（待 FinBERT 分析）"""
    return _redis_service.pending_finbert_push(news_id, table_name)


def pending_finbert_push_batch(news_ids: list, table_names: list = None) -> int:
    """批量推入待 FinBERT 分析的 ID"""
    return _redis_service.pending_finbert_push_batch(news_ids, table_names)


def pending_finbert_pop_batch(count: int = 16) -> list:
    """原子批量获取并删除 news:pending_finbert 中最多 count 个项"""
    return _redis_service.pending_finbert_pop_batch(count)


def pending_finbert_size() -> int:
    """获取 news:pending_finbert List 的长度"""
    return _redis_service.pending_finbert_size()
...
__all__ = [
    ...
    # Layer 2.5: news:pending_finbert
