...
def get_news_table_ddl(table_name: str) -> str | None:
    """
    根据实际表名生成新闻分表完整 DDL。

    由于新闻表按月分表（news_xxx_YYMM），直接填入表名。
    注意：这里不包含 table_comment，调用方如需要可自行追加。
    """
    if not table_name.startswith("news_"):
        return None
    return CREATE_NEWS_TABLE_DDL.format(table_name=table_name, comment="新闻")
