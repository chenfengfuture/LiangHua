    ...
    def _get_or_load_table_info(self, table_name: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if cache_entry and time.time() - cache_entry.get("timestamp", 0) < self.cache_ttl_seconds:
                if cache_entry.get("columns"):
                    return cache_entry
                else:
                    logger.warning(f"表 {table_name} 的缓存字段集为空，强制重新加载")
            # 加载表结构
        ...
        try:
            with get_cursor() as cur:
                ...
                if not columns:
                    ...
                    self._cache.pop(table_name, None) # 清除内存中的缓存
                    time.sleep(1)
                    # 表创建后，重新查询一次表结构
                    with get_cursor() as new_cur:
                        if use_explicit_schema:
                            new_cur.execute(query, (schema, tbl))
                        else:
                            new_cur.execute(query, (tbl,))
